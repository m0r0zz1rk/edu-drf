...
class StudentGroupDocsViewSet(DocsViewSet):
    ...
    update_serializer = StudentGroupOfferSerializer
    print_file_serializer = PrintFileSerializer
    ...
    @swagger_auto_schema(
        ...
        request_body=print_file_serializer,
        responses={
    ...
    def print_file(self, request, *args, **kwargs):
        serialize = self.print_file_serializer(data=request.data)
        if serialize.is_valid():

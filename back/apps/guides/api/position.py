...
from apps.guides.selectors.position import position_queryset, position_orm
from apps.guides.serializers.position import PositionListUpdateSerializer, PositionBaseSerializer
from apps.journal.consts.journal_modules import GUIDES


class PositionViewSet(GuideViewSet):
    orm = position_orm
    queryset = position_queryset()
    serializer_class = PositionListUpdateSerializer
    base_serializer = PositionBaseSerializer
    create_serializer = PositionBaseSerializer
    update_serializer = PositionListUpdateSerializer
    filterset_class = NameFieldFilter
    swagger_object_name = 'Должность'

<template>
  ...
  >
    ...
    <v-expansion-panel
      v-if="!([undefined, null, 0].includes(groupInfo.plan_seats_number))"
    ...
    >
      <v-expansion-panel-text>
        <v-number-input
          ...
          :min="1"
          v-model="groupInfo.plan_seats_number"
          :loading="loading"
        />
      </v-expansion-panel-text>
    </v-expansion-panel>
...
export default {
  ...
  methods: {
    ...
    curatorSelect(curator) {
      ...
      this.groupInfo.curator_id = curator.object_id

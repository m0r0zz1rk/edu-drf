<template>
  ...
  >
    ...
    <v-radio-group v-model="referralRadio">
      ...
      <v-radio
          v-if="appType"
          :value="`информационное письмо о проведении ${appType === 'ou' ? 'обучения' : 'курса'}`"
          :label="`информационное письмо о проведении ${appType === 'ou' ? 'обучения' : 'курса'}`"
      />
      <v-radio
  ...
  data() {
    return {
      ...
      possibleReferral: [
          ...
          'госпаблик ВКонтакте',
          `информационное письмо о проведении ${this.appType === 'ou' ? 'обучения' : 'курса'}`,
          'почтовая рассылка',

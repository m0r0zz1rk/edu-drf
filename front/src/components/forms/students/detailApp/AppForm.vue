<template>
  ...
  >
    ...
    <v-radio-group v-model="referralRadio">
      ...
      <v-radio
          v-if="appType"
          :value="`информационное письмо о проведении ${appType === 'ou' ? 'обучения' : 'семинара'}`"
          :label="`информационное письмо о проведении ${appType === 'ou' ? 'обучения' : 'семинара'}`"
      />
      <v-radio
  ...
  data() {
    return {
      ...
      possibleReferral: [
          ...
          'госпаблик ВКонтакте',
          `информационное письмо о проведении ${this.appType === 'ou' ? 'обучения' : 'семинара'}`,
          'почтовая рассылка',
      ...
      payOptions: [
        {key: true, title: 'Физическое лицо (оплата из собственных средств)'},
        {key: false, title: 'Юридическое лицо (оплата за счёт средств работодателя)'}
      ],

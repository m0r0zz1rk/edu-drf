      ...
      <DialogContentWithError ref="content-error">
        ...
        <slot>
          ...
          <CokoDialog ref="agreementDialog">
            ...
            <template v-slot:text>
              <p>
                ...
                - Дата рождения;<br/>
                - Пол.<br/>
                Настоящее согласие, выданное мной ГАУ ИО ЦОПМКиМКО, действует до момента удаления моей учетной
                записи в АИС, либо до момента прекращения ГАУ ИО ЦОПМКиМКО эксплуатации АИС, если иное не предусмотрено
                ...
                направления мною либо моим представителем соответствующего письменного запроса (заявления) в ГАУ
                ИО ЦОПМКиМКО по адресу: 664023, Иркутская область, город Иркутск, улица Лыткина, строение 75/1.<br/>
          ...
          <MailingDialog
              ...
              :onMailingClose="() => {mailingIsOpen = true}"
          />
        </slot>
...
export default {
  ...
  data() {
    return {
      ...
      // Окно с формой согласие на рассылку было открыто
      mailingIsOpen: false,
      // Рассылка по email
      emailMailing: null,
  ...
  methods: {
    ...
    verifyData() {
      ...
      if (this.emailMailing === null || this.phoneMailing === null) {
        if (!this.mailingIsOpen) {
          this.$refs["mailingDialog"].openMailingDialog()
          this.mailingIsOpen = true
          return null
        } else {
          this.emailMailing = this.emailMailing !== null ? this.emailMailing : true
          this.phoneMailing = this.phoneMailing !== null ? this.phoneMailing : true
        }
      }
      return {
        'surname': surname,
